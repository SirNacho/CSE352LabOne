...
void timer_init(uint8_t duration) {
    ...
    uint32_t fTimer = 16000000 / (1 << prescaler); // FILL THIS IN
    
    // TODO: Calculate number of ticks for desired duration
    // N = duration * fTimer
    uint32_t N = duration * fTimer; // FILL THIS IN
    
    // TODO: Enable COMPARE[0] interrupt
    // Hint: Set bit 16 in INTENSET register
    NRF_TIMER4->INTENSET = 1 << 16; // FILL THIS IN
    ...
    NRF_TIMER4->PRESCALER = prescaler; // FILL THIS IN
    
    // TODO: Set bit mode (24-bit = 2)
    NRF_TIMER4->BITMODE = 2; // FILL THIS IN
    
    // TODO: Set timer mode (Timer = 0)
    NRF_TIMER4->MODE = 0; // FILL THIS IN
    
    // TODO: Set compare value
    NRF_TIMER4->CC[3] = N; // FILL THIS IN
...
void counter_init() {
    // TODO: Set bit mode (24-bit = 2)
    NRF_TIMER3->BITMODE = 2; // FILL THIS IN
    
    // TODO: Set counter mode (Counter = 1)
    NRF_TIMER3->MODE = 1; // FILL THIS IN
...
void init_pwm(uint16_t pwm, uint32_t prescalar, uint32_t counterTop) {
    printf("Intializing pwm...\n");
    printf("PWM %d, prescalar: %d, counterTop %d\n", pwm, prescalar, counterTop);
    uint16_t pwm_seq[4] = {(counterTop * 0.1), (counterTop * 0.25), (counterTop * 0.5), (counterTop * 0.75)};

    NRF_PWM0->PSEL.OUT[0] = (11 << PWM_PSEL_OUT_PIN_Pos) | (PWM_PSEL_OUT_CONNECT_Connected << PWM_PSEL_OUT_CONNECT_Pos);

    NRF_PWM0->ENABLE = (1 << PWM_ENABLE_ENABLE_Pos);
    NRF_PWM0->MODE = (1 << PWM_MODE_UPDOWN_Pos);
    NRF_PWM0->PRESCALER = (0 << prescalar);
...
void GPIOTE_IRQHandler(void) {
    ...
    if (NRF_GPIOTE->EVENTS_IN[0]) { // FILL THIS IN - Check NRF_GPIOTE->EVENTS_IN[0]
        ...
        if (gpio_read(22) == 0) { // FILL THIS IN - Use gpio_read(22) == 0
            // TODO: Increase counterTop
            // FILL THIS IN
            counterTop = counterTop + 100;
    ...
    if (NRF_GPIOTE->EVENTS_IN[1]) { // FILL THIS IN - Check NRF_GPIOTE->EVENTS_IN[1]
        printf("This suppose to count\n");
...
void TIMER4_IRQHandler(void) {
    ...
    printf("Timer4 Interrupt Recieved!\n");
    ...
    lastCount = read_counter(); // FILL THIS IN - Use read_counter()
...
int main(void) {
  ...
  printf("Display initialized\n");
    ...
    NRF_GPIOTE->CONFIG[0] |= (28 << GPIOTE_CONFIG_PSEL_Pos);
    ...
    NRF_GPIOTE->CONFIG[1] |= (13 << GPIOTE_CONFIG_PSEL_Pos);
